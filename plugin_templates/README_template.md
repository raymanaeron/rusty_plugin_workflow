...
### Topics
- `{{resource_name_camel}}Updated` - Published when resource changes
- `{{resource_name_camel}}Completed` - Published when plugin operation completes
...
export async function activate(container, appManager) {
  ...
  function publishCompletion() {
    const published = appManager.publish(
      ...
      '{{resource_name_camel}}Completed',  // Topic
...
export async function activate(container, appManager) {
  ...
  appManager.subscribe('{{plugin_name}}', '{{resource_name_camel}}Updated', (data) => {
