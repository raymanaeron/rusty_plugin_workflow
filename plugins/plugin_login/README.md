...
### PATCH /api/login/userprofile/{id}
...
### DELETE /api/login/userprofile/{id}
...
### Topics
- `UserprofileUpdated` - Published when resource changes
- `UserprofileCompleted` - Published when plugin operation completes
...
export async function activate(container, appManager) {
  ...
  function publishCompletion() {
    const published = appManager.publish(
      'plugin_login',                   // Publisher name
      'UserprofileCompleted',  // Topic
    ...
    RUNTIME.spawn(async move {
        if let Ok(mut ws_client) = client.lock() {
            ...
            if let Err(e) = ws_client.publish(
                ...
                eprintln!("[plugin_login] Failed to publish: {}", e);
            } else {
                println!("[plugin_login] Successfully published completion event");
            }
        }
    });
}
...
export async function activate(container, appManager) {
  ...
  appManager.subscribe('plugin_login', 'UserprofileUpdated', (data) => {
...
pub async fn create_ws_plugin_client() {
    if let Ok(client) = WsClient::connect("plugin_login", "ws://127.0.0.1:8081/ws").await {
        ...
        if let Ok(mut ws_client) = client.lock() {
            ...
            ws_client.on_message("UserprofileUpdated", |msg| {
                println!("[plugin_login] Received update: {}", msg);
...
└── plugin_login/
    ...
    ├── web/
    │   ├── step-login.html  # HTML UI template
    │   ├── step-login.js    # JavaScript logic
