...
export async function activate(container, appManager, jwtManager) {
    ...
    appManager.registerPlugin('plugin_mockwifi');
    ...
    function getSignalIconName(signal) {
        ...
        if (signal >= -50) return "/mwifi/web/icons/wifi-strong.svg";
        if (signal >= -60) return "/mwifi/web/icons/wifi-good.svg";
        if (signal >= -70) return "/mwifi/web/icons/wifi-fair.svg";
        return "/mwifi/web/icons/wifi-weak.svg";
    }
    ...
    async function getNetworkList() {
        try {
            ...
            networkList.innerHTML = "";
              const response = await jwtManager.secure_request('/api/mwifi/network');
            
            if (!response.ok) throw new Error(`Network scan failed (${response.status})`);
            
            const networksData = await response.json();
            
            // Convert the networks object to an array
            const networks = Object.entries(networksData).map(([id, network]) => ({
                id,
                ...network
            }));
    ...
    connectBtn.addEventListener("click", async () => {
        ...
        try {
            const res = await jwtManager.secure_request("/api/mwifi/network", {
                method: "POST",
            ...
            if (res.ok) {
                ...
                const published = appManager.publish('plugin_mockwifi', 'WifiCompleted', 
    ...
    return () => {
        appManager.unregisterPlugin('plugin_mockwifi');
    };
}
